...
public class LogonController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		reg.setOnAction(e -> {
			...
			chkFlag = chk(char_id, id, "아이디 조건 알림", "아이디는 영문(소문자) 또는 숫자로만 이루어질 수 있습니다.", "아이디 입력 필수",
					"아이디는 공백일 수 없습니다.", "아이디 다시 입력", "아이디는 12자 이내로 작성해주세요."); // id 체크 완료

			if (chkFlag)
				return; // id 문제 발생하면 종료

			chkFlag = chk(char_passwd, passwd, "비밀번호 조건 알림", "비밀번호는 영문(소문자) 또는 숫자로만 이루어질 수 있습니다.", "비밀번호 입력 필수",
					"비밀번호는 공백일 수 없습니다.", "비밀번호 다시 입력", "비밀번호는 12자 이내로 작성해주세요."); // 비밀번호 체크 완료

			if (chkFlag)
				return; // 비밀번호 문제 발생하면 종료
